...
function loadFonts() {
    ...
    fontsToDisplay.forEach(fontData => {
        ...
        fontDiv.innerHTML = `
            ...
            <div class="subtle-text">${fontData.family} mozete preuzeti <a href="${fontData.files.regular}" class="download-link" download>ovde</a></div>
            </div>
        `;
...
function updateText() {
    enteredText = document.getElementById('text-input').value;
    loadFonts();
}
